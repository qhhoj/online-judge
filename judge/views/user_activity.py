...
def user_activity_detail(request, username):
    ...
    ip_stats = UserActivity.objects.filter(**filter_condition).values('ip_address').annotate(
        count=Count('id'),
    ).order_by('-count')[:20]

    # Thống kê theo path được truy cập nhiều nhất - Lịch sử đầy đủ
    path_stats = UserActivity.objects.filter(**filter_condition).values('path').annotate(
        count=Count('id'),
    ).order_by('-count')[:30]
...
def export_logs(request):
    ...
    response['Content-Disposition'] = f'attachment; filename="user_activity_logs_{timestamp}.csv"'
...
def delete_anonymous_logs(request):
    ...
    if request.method == 'POST':
        ...
        messages.success(
            request,
            f'Đã xóa {deleted_activities} activities và {deleted_sessions} sessions của anonymous users',
        )
