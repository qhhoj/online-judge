...
from judge.utils.recaptcha import (
    ReCaptchaField,
    ReCaptchaWidget,
)
from judge.utils.subscription import (
    Subscription,
    newsletter_id,
)
...
class CustomRegistrationForm(RegistrationForm):
    username = forms.RegexField(
        regex=re.compile(r'^\w+$', re.ASCII), max_length=30, label=_('Username'),
        error_messages={
            'invalid': _(
                'A username must contain letters, '
    ...
    timezone = ChoiceField(
        label=_('Timezone'), choices=TIMEZONE,
        widget=Select2Widget(attrs={'style': 'width:100%'}),
    )
    language = ModelChoiceField(
        queryset=Language.objects.all(), label=_('Preferred language'), empty_label=None,
        widget=Select2Widget(attrs={'style': 'width:100%'}),
    )
    ...
    def clean_email(self):
        ...
        if '@' in self.cleaned_data['email']:
            ...
            if (
                domain in settings.BAD_MAIL_PROVIDERS or
