...
def submit_external_submission(
    self, submission_id, rejudge=False, batch_rejudge=False, expected_pcd_submission_id=None,
):
    try:
...
def poll_external_submission(self, submission_id, expected_pcd_submission_id=None):
    ...
    if external_submission_is_rejected(submission, ext_sub):
        return
    if submission.status in ('D', 'CE', 'IE', 'AB', 'RJ') and ext_sub.pcd_system_status not in ('queued', 'polling'):
        return
