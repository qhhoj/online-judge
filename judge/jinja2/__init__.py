    ...
    return mark_safe(f"""
        <tr class="mb-4">
            <th style="max-width: 10em; width: 10em">
                <label for="{field.auto_id}" class="text-base font-bold'
                ' text-gray-900 {'need' if field.field.required else ''}">{field.label}:</label>
            </th>
            <td class="pl-5">
                {form_input}
                {help_text}
...
def render_text_input(field):
    attrs = {
        ...
        ' rounded-lg focus:ring-primary-600 focus:border-primary-600 w-2/5 p-2.5',
    }
...
def render_file_input(field):
    attrs = {
        'class': 'w-2/5 text-sm text-gray-900 border border-gray-300'
