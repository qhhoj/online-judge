...
class Ticket(models.Model):
    ...
    user = models.ForeignKey(
        Profile, verbose_name=_('ticket creator'), related_name='tickets',
    ...
    notes = models.TextField(
        verbose_name=_('quick notes'), blank=True,
        help_text=_('Staff notes for this issue to aid in processing.'),
    )
    content_type = models.ForeignKey(
        ContentType, verbose_name=_('linked item type'),
        on_delete=models.CASCADE,
    )
...
class TicketMessage(models.Model):
    ticket = models.ForeignKey(
        Ticket, verbose_name=_('ticket'), related_name='messages',
        related_query_name='message', on_delete=models.CASCADE,
    )
    user = models.ForeignKey(
        Profile, verbose_name=_('user'), related_name='ticket_messages',
        on_delete=models.CASCADE,
    )
